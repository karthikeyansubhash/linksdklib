...
/**
 * <p>Implementations of this class provide functions to create Workpath authentication agent for integrating with device authentication.</p>
 * <p>The Workpath authentication agent implementor can create 3 types of authentication services :
 * <ul>
 * <li>Prompt</li>
 * <li>Preprompt</li>
 ...
 * </p>
 * <p>
 * Prompt type needs to implement an "Activity" to show the sign in form and has to configure "Activity" name in hpk file using hpktool(in SDK Packages).
 * Preprompt type needs to receive onPrePrompt() event and has to be checked as preprompt option in hpk file.
 * </p>
 * <p>
 * When authentication service is started, Workpath platform checks Authentication Agent types which is included in installation option from hpk configuration and decide the next following steps :
 * <ul>
 ...
 * </p>
 * <p>
 * A Workpath authentication agent which has Preprompt will call {@link AccessService AccessService}.signIn() method with {@link SignInAction SignInAction} to decide authentication process.
...
public abstract class AbstractAuthenticationService extends Service {
            ...
            switch (msg.what) {
                ...
                case AuthMessage.SIGN_IN:
                    synchronized (serviceBase.mLock) {
                        ...
                        com.hp.jetadvantage.link.api.access.Principal principal = data.getParcelable(Accesslet.PRINCIPAL_EXTRA);
                        serviceBase.onSignIn(Principal.CREATOR_OBJ.createFromObject(principal));
                    }
                    break;
     ...
     * Workpath Authentication Agent can decide actions between {@link SignInAction SignInAction} before starting prompt activity.</p>
     *
     * @since API 2
     */
