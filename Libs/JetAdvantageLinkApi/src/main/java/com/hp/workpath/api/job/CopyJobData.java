...
public class CopyJobData implements JobInfo.JobData {
    ...
    @SuppressLint("RestrictedApi")
    private CopyJobData(Object object) {
        if(object instanceof com.hp.jetadvantage.link.api.job.CopyJobData) {
            mVersion = ((com.hp.jetadvantage.link.api.job.CopyJobData) object).getVersion();
            Preconditions.checkArgument(mVersion >= Sdk.VERSION_LEVEL.ONE);

            mImagesScanned = ((com.hp.jetadvantage.link.api.job.CopyJobData) object).getA();
            mSheetsPrinted = ((com.hp.jetadvantage.link.api.job.CopyJobData) object).getB();
            mDuplex = CopyAttributes.Duplex.valueOf(((com.hp.jetadvantage.link.api.job.CopyJobData) object).getC().name());
            mScanSize = CopyAttributes.ScanSize.valueOf(((com.hp.jetadvantage.link.api.job.CopyJobData) object).getD().name());

            mJobState = CopyJobState.CREATOR_OBJ.createFromObject(((com.hp.jetadvantage.link.api.job.CopyJobData) object).getE());
            mJobExecutionMode = CopyAttributes.JobExecutionMode.valueOf(((com.hp.jetadvantage.link.api.job.CopyJobData) object).getF().name());
        }
    }
     ...
     * @hide trivial
     */
    public static final Convertor.ConvertorCreator<CopyJobData> CREATOR_OBJ = new Convertor.ConvertorCreator<CopyJobData>() {
        @Override
        public CopyJobData createFromObject(Object object) {
            return new CopyJobData(object);
        }
    };
