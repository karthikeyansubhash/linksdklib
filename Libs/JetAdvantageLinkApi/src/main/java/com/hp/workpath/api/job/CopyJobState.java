 ...
 * @since API 3
 */
public class CopyJobState implements JobInfo.JobState, Convertor {
    /**
    ...
    private CopyJobState(Object object) {
        if(object instanceof com.hp.jetadvantage.link.api.job.CopyJobState) {
            mState = State.valueOf(((com.hp.jetadvantage.link.api.job.CopyJobState) object).getA().name());
            mScanningState = ActivityState.valueOf(((com.hp.jetadvantage.link.api.job.CopyJobState) object).getB().name());
            mProcessingState = ActivityState.valueOf(((com.hp.jetadvantage.link.api.job.CopyJobState) object).getC().name());
            mPrintingState = ActivityState.valueOf(((com.hp.jetadvantage.link.api.job.CopyJobState) object).getD().name());
            mCancelingState = ActivityState.valueOf(((com.hp.jetadvantage.link.api.job.CopyJobState) object).getE().name());
        }
    }
     ...
     * @hide trivial
     */
    public static final ConvertorCreator<CopyJobState> CREATOR_OBJ = new ConvertorCreator<CopyJobState>() {
        @Override
        public CopyJobState createFromObject(Object object) {
            return new CopyJobState(object);
        }
    };
