 ...
 * @since API 1
 */
public class ScanJobState implements JobInfo.JobState, Convertor {
    ...
    private ScanJobState(Object object) {
        if(object instanceof com.hp.jetadvantage.link.api.job.ScanJobState) {
            mState = State.valueOf(((com.hp.jetadvantage.link.api.job.ScanJobState) object).getE().name());
            mScanningState = ActivityState.valueOf(((com.hp.jetadvantage.link.api.job.ScanJobState) object).getA().name());
            mProcessingState = ActivityState.valueOf(((com.hp.jetadvantage.link.api.job.ScanJobState) object).getB().name());
            mTransmittingState = ActivityState.valueOf(((com.hp.jetadvantage.link.api.job.ScanJobState) object).getC().name());
            mCancelingState = ActivityState.valueOf(((com.hp.jetadvantage.link.api.job.ScanJobState) object).getD().name());
        }
    }
     ...
     * @hide trivial
     */
    public static final ConvertorCreator<ScanJobState> CREATOR_OBJ = new ConvertorCreator<ScanJobState>() {
        @Override
        public ScanJobState createFromObject(Object object) {
            return new ScanJobState(object);
        }
    };
